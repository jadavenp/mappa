...
End-to-end verified in Safari against `npm run preview` (see `.superpowers/sdd/task-4-report.md` for full evidence): `bake/` tests 28/28, timeline scrub sweep across 1920/1935/1950/1963.9/1964.5/1970/1975 matches `public/v0/timeline.json` exactly (visible-mesh count grows to 15, drops by 4 at the 1964 quake, recovers to 14 by 1970), building extrusions sit base-at-ground with correct height, and the default camera is centered on the town at the region's configured height/heading/pitch. Screenshots could not be captured in this pass — the automation shell (Terminal.app) lacks the macOS Screen Recording permission, so `screencapture` returns black frames; grant it under System Settings → Privacy & Security → Screen Recording, relaunch Terminal, and re-run to populate `docs/verify/`.
